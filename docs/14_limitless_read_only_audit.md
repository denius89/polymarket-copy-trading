...
Limitless технически подходит для публичного рейтинга трейдеров, исторического анализа и paper-trading spike. Площадку пока нельзя утверждать для автоматического копирования и нельзя считать готовой заменой Polymarket для Сомали и Таиланда.

Три блокера до выбора Limitless как execution-площадки:
...
Фактическая комиссия приходит в execution response как `effectiveFeeBps`, `usdFee` и `contractsFee`. Наши настраиваемые 1% с buy и sell являются дополнительной комиссией. В неблагоприятном CLOB-примере совокупная номинальная нагрузка по двум сторонам может приблизиться к 6,5% до спреда и проскальзывания: 3% + 1% на входе и 1,5% + 1% на выходе. Точный расчёт должен использовать реальные базы каждой операции.
...
Limitless остаётся в shortlist для всех пяти GEO и особенно для проверки Сомали и Таиланда. Ближайший допустимый этап — 72-часовой read-only ingestion/latency spike и запрос в partner program. Создание аккаунтов, получение partner scopes, хранение ключей и отправка ордеров пока не требуются.

Для малого бюджета и полной автоматизации Polymarket пока остаётся первым техническим кандидатом. Если Limitless подтвердит GEO, коммерческое использование данных и безопасный delegated flow без withdrawal, его можно сравнить с Polymarket по четырём измеримым критериям: p95 задержки, доля копируемых сделок на $10/$50/$200, полная комиссия round trip и качество recovery/revocation.
