...
17. Сервисная комиссия возникает только у подтверждённого автоматического fill; неисполненный остаток и отмена не создают комиссию.
18. Manual, protective, emergency и error-correction fills сохраняются с нулевой сервисной ставкой и явным reason code.
19. Виртуальная комиссия альфы не создаёт денежное списание или партнёрское начисление.
20. Partner accrual проходит `PENDING → AVAILABLE → PAID` либо корректирующее `REVERSED`; повтор fee event не создаёт второе начисление.
21. Истечение 24-месячной атрибуции прекращает новые начисления, но не отменяет уже заработанные; пожизненный срок возможен только в версионируемом VIP-профиле.
