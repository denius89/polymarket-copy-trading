...
Polymarket и Limitless являются двумя обязательными площадками продукта. Общий core проектируется venue-neutral, а доступ к данным, кошелькам, ордерам, комиссиям и settlement реализуется отдельными adapters. Limitless идёт первым партнёрским spike; Polymarket Builder/Session Keys проверяются параллельно. Paper mode должен покрыть обе площадки, а live execution включается для каждой отдельно после её gates.
...
Интерфейс с первого компонента строится на i18n-слое: стабильные ключи вместо зашитого текста, locale-aware форматирование чисел, дат и валют, ручной выбор языка и поддержка RTL. Reason codes и торговые состояния остаются языконезависимыми. Конкретный launch-набор языков определяется после выбора GEO; правила зафиксированы в ADR-0006.
...
| `FeePolicyVersion` | неизменяемые maker/taker ставки, этап, исключения и дата действия |
| `ServiceFeeEvent` | фактическая или виртуальная комиссия конкретного fill с reason code |
| `ReferralAttribution` | партнёр, клиент, срок 24 месяца или VIP-исключение |
| `PartnerAccrual` | pending/available/paid/reversed начисление от полученной комиссии |
...
11. Классифицировать automatic/manual/protective и maker/taker; применить версию тарифа только к фактическому fill. В альфе записать виртуальную комиссию без денежного события.
12. Создать партнёрское начисление, если сервисная комиссия фактически получена и атрибуция действует.
13. Записать итог, задержку, цену, проскальзывание, комиссии и причину результата в журнал.
...
Limitless выбран первой интеграционной проверкой по ADR-0007. EOA-подпись и партнёрские managed wallets имеют разные границы контроля. Для полной автоматизации основной кандидат — sub-account с `delegated_signing`; токен execution worker получает `trading`, `account_creation` и `delegated_signing`, но не `withdrawal`. HMAC secret хранится только на backend. Trust, recovery и withdrawal path остаются блокерами; полный checklist находится в документе 20.

Доступ к Polymarket разделён на публичные API без credentials, пользовательские CLOB L1/L2 credentials и проектный Builder profile. Для управления beta Session Keys Builder API key должен быть отдельно разрешён Polymarket. Пошаговый план запроса, tiers, секреты и технические проверки описаны в `19_polymarket_api_access.md`.
...
Рекомендация для будущего технического spike — сначала встроенный Limitless/Privy server-wallet flow, затем EOA как контроль границы доверия. Отдельный выбор Privy/Dynamic/Turnkey остаётся актуальным для собственного onboarding-слоя или будущего Polymarket adapter. Проверяются вход, sub-account creation, delegated signing, token revocation, recovery и вывод при недоступности нашего backend.
...
### Spike E: Limitless — первый приоритет

Отдельно проверить публичную историю адресов, WebSocket, order lifecycle, fee model и возможность безопасной авторизации. Результат определит ограничения обязательного Limitless adapter и порядок его live-включения.
